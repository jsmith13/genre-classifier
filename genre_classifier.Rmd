...
# import the dataset
music <- read.csv("final_music_descriptors.csv")
...
save(train.set, file = "Objects/train.set")
save(test.set, file = "Objects/test.set")
```
...
save(glm.training.results, file = "Objects/glm.training.results")
```
...
k.values <- seq(1,101,5)
distances <- c(seq(0.5, 1, 0.25), seq(2, 4, 1))
...
# save the tuning results
save(knn.training.results, file = "Objects/knn.training.results")
```
```{r knn tune results}
...
# this "orders" the parameters k, kernels, distance
...
knn.tuning.scores <- cbind(expand.grid(k.values, kernels, distances), knn.tuning.scores)
colnames(knn.tuning.scores) <- c("k", "kernel", "distance", "score")

# plot a heatmap of classification score as a function of kernel and k, facet by Minkowski parameter
ggplot(knn.tuning.scores, aes(k, distance, fill = score)) + geom_raster() + scale_y_discrete("distance") +
  scale_x_continuous("k", breaks = seq(1, 101, 10)) + facet_wrap(~kernel) + ggtitle("KNN Tuning Parameters")
```
...
save(optimal.knn, file = "Objects/optimal.knn")
```
...
save(b.tuning.scores, file = "Objects/b.tuning.scores")
...
# save the tuning scores
save(rf.tuning.scores, file = "Objects/rf.tuning.scores")
...
# save the optimized model
save(optimal.rf, file = "Objects/optimal.rf")
```
